...
#ifndef __LSM6DSMIMU_H__
#define __LSM6DSMIMU_H__
#define SPI_MODE 1
#include "stdint.h"
...
private:
	...
	const uint32_t SPI_CLOCK = 10000000; // 8 MHz default, 10 max
...
public:
	...
	LSM6DS3( uint8_t busType = SPI_MODE, uint8_t inputArg = 22 );
	~LSM6DS3() = default;
	...
	status_t read_all_temp(float* ax, float* ay, float* az, float* gx, float* gy, float* gz, float* temp);
	status_t read_all(float* ax, float* ay, float* az, float* gx, float* gy, float* gz);
	...
	void setCalibration (float * gyroBias, float * accelBias);

	bool enable_motion_detection( uint8_t level );
	bool enable_normal_mode();
	bool enable_sleep();
	bool enable_dri();
	bool disable_dri();
	bool enable_fifo();
	int write_check_reg(uint8_t adress, uint8_t data);
	void reset();
