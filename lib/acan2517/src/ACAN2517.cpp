...
static const uint16_t C1VEC_REGISTER      = 0x018 ; // interrupts
...
//----------------------------------- CS and INT pins
 ...
 //----------------------------------- Wait (2 ms max) until requested mode is reached
  bool wait = true ;
  uint8_t actualMode;
  uint32_t deadline = millis () + 4 ;
  while (wait) {
    actualMode = (readRegister8 (C1CON_REGISTER + 2) >> 5) & 0x07 ;
...
bool ACAN2517::sleep2517FD (void) {
  // INT0 and INT1 needs to be high for low power mode

if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x01){ // read OPMOD and check if already in sleep mode
  writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
  writeRegister8 (IOCON_REGISTER    , 0x03) ; // Set INT0 and INT1 as OUTPUT
  writeRegister8 (IOCON_REGISTER + 1, 0x03) ; // Set GPIO0 and 1 High
  uint8_t osc = readRegister8(OSC_REGISTER); // do not edit clock divider
  writeRegister8 (OSC_REGISTER      , osc | 0x04 ) ; // disable OSCDIS Oscillator
  delay(2);
  writeRegister8 (C1CON_REGISTER + 3, 0x01 | (1 << 3)) ; // Request sleep mode, abort all transmissions
  delay(5); // wait 5 ms
  if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x01){
    return true; // check if now in sleep mode
  } else { return false;} // if not, return error
}
return true;

}

bool ACAN2517::en_wake_on_can2517FD (void) {
  // INT0 high and INT1 low for low power mode
  writeRegister8 (C1CON_REGISTER + 3, 0x04 | (1 << 3)); // request configuration mode, abort all messages
  delay(2);
  if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x04){ // if is in configuration mode
    writeRegister8 (C1CON_REGISTER + 1, 0x05); // enable wake on can and set filter to B10
    writeRegister8 (C1VEC_REGISTER + 3, 0x42) ; // Wakeup interrupt
    writeRegister8 (C1INT_REGISTER + 3, 0x40) ; // en Wakeup interrupt

    writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
    writeRegister8 (IOCON_REGISTER    , 0x03) ; // Set INT0 and INT1 as OUTPUT
    writeRegister8 (IOCON_REGISTER + 1, 0x01) ; // Set GPIO0 high and 1 Low 0x01
    uint8_t osc = readRegister8(OSC_REGISTER); // do not edit clock divider
    writeRegister8 (OSC_REGISTER      , osc | 0x04 ) ; // disable OSCDIS Oscillator
    delay(2);
    writeRegister8 (C1CON_REGISTER + 3, 0x01 | (1 << 3)) ; // Request sleep mode, abort all transmissions
...
bool ACAN2517::dis_wake_on_can2517FD (void) {
  // INT0 high and INT1 low for low power mode
 //   writeRegister8 (C1CON_REGISTER + 3, 0x04 | (1 << 3)); // request configuration mode, abort all messages
 //   delay(2);
 //   if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x04){ // if is in configuration mode
 //     writeRegister8 (C1CON_REGISTER + 1, 0x05); // enable wake on can and set filter to B10
 //     writeRegister8 (C1INT_REGISTER + 3, 0x00) ; // disable all interrupts
//  
 //     //writeRegister8 (C1VEC_REGISTER, 0x42) ; // Wakeup interrupt
//  
 //     writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
 //     writeRegister8 (IOCON_REGISTER    , 0x03) ; // Set INT0 and INT1 as OUTPUT
 //     writeRegister8 (IOCON_REGISTER + 1, 0x00) ; // Set GPIO0 high and 1 Low 0x01
 //     uint8_t osc = readRegister8(OSC_REGISTER); // do not edit clock divider
 //     writeRegister8 (OSC_REGISTER      , osc | 0x04 ) ; // disable OSCDIS Oscillator
 //     delay(2);
 //     writeRegister8 (C1CON_REGISTER + 3, 0x01 | (1 << 3)) ; // Request sleep mode, abort all transmissions
...
bool ACAN2517::set_listen_only2517FD (void) {
  // INT0 high and INT1 low for low power mode
  writeRegister8 (C1CON_REGISTER + 3, 0x04 | (1 << 3)); // request configuration mode, abort all messages
  delay(2);
  if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x04){ // if is in configuration mode

    writeRegister8 (C1CON_REGISTER + 3, 0x03 | (1 << 3)) ; // Request listen only mode, abort all transmissions
    delay(5); // wait 5 ms
    if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x03){
      return true; // check if now in sleep mode
    }
  }
  return false; // if not, return error
...
bool ACAN2517::set_normal_mode2517FD (void) {
  // INT0 high and INT1 low for low power mode
  writeRegister8 (C1CON_REGISTER + 3, 0x04 | (1 << 3)); // request configuration mode, abort all messages
  delay(2);
  if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x06){ // if is in configuration mode

    writeRegister8 (C1CON_REGISTER + 3, 0x06 | (1 << 3)) ; // set CAN 2.0B mode (not FD), abort all transmissions
    delay(5); // wait 5 ms
    if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x06){
      return true; // check if now in normal mode
    }
  }
  return false; // if not, return error
return true;
...
bool ACAN2517::wake2517FD (void) {
  // INT0 and INT1 needs to be high for low power mode
  uint8_t osc = readRegister8(OSC_REGISTER); // do not edit clock divider
  uint32_t errorCode = 0;
  //writeRegister8 (OSC_REGISTER      , osc ^ 0x04 ) ; // enable OSCDIS Oscillator
  osc &= ~(1UL << 2); // set 2nd bit zero = enable osc
  ...
  while (wait) {
    wait = (readRegister8 (OSC_REGISTER + 1) & 0x4) == 0 ;  // DS20005688B, page 16 - get OSCRDY (clock is running and stable)
    if(wait){writeRegister8 (OSC_REGISTER,osc);}
    if (wait && (millis() >= deadline)) {
      errorCode = kX10PLLNotReadyWithin1MS ;
      wait = false ;
    }
  }
 // Serial.println(readRegister8(OSC_REGISTER));
 // Serial.println(readRegister8(OSC_REGISTER+1));
  //Serial.println(readRegister8(C1CON_REGISTER+2));

// if(readRegister8(C1CON_REGISTER + 2) | 0xE0 == 0x01){ // read OPMOD and check if is in sleep mode

//   writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
//   writeRegister8 (IOCON_REGISTER    , 0x03) ; // Set INT0 and INT1 as OUTPUT
//   writeRegister8 (IOCON_REGISTER + 1, 0x000) ; // Set GPIO0 and 1 LOW
//   writeRegister8 (C1CON_REGISTER + 3, 0x04 | (1 << 3)) ; // Request configuration mode, abort all transmissions
//   delay(5); // wait 5 ms
...
//  if(readRegister8(C1CON_REGISTER + 2) | 0xE0 != 0x04){
 //   return true; // check if now in configuration mode
 // } else { return false;} // if not, return error
...
bool ACAN2517::en_transceiver2517FD (void) {
  // INT0 and INT1 needs to be low for operational mode
  writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
  writeRegister8 (IOCON_REGISTER    , 0x00) ; // Set INT0 and INT1 as OUTPUT
  writeRegister8 (IOCON_REGISTER + 1, 0x00) ; // Set GPIO0 and GPOI1 low
  //writeRegister8 (IOCON_REGISTER , 0x03000000); //0x00 | (0x00<<8) | (0x03 <<24)) ; // Set GPIO0 and GPOI1 low

  if(readRegister8(IOCON_REGISTER + 1) | 0xFF != 0x00){
    return true; // check if now in configuration mode
  } else { return false;} // if not, return error
...
bool ACAN2517::dis_transceiver2517FD (void) {
  // INT0 and INT1 needs to be low for operational mode
  writeRegister8 (IOCON_REGISTER + 3, 0x03) ; // Set INT0 and INT1 as GPIO
  writeRegister8 (IOCON_REGISTER    , 0x00) ; // Set INT0 and INT1 as OUTPUT
  writeRegister8 (IOCON_REGISTER + 1, 0x03) ; // Set GPIO0 and GPOI1 high
  //writeRegister8 (IOCON_REGISTER , 0x03030300);// 0x00 | (0x03<<8) | (0x03 <<24)) ; // Set GPIO0 and GPOI1 high

  if(readRegister8(IOCON_REGISTER + 1) | 0xFF != 0x03){
    return true; // check if now in configuration mode
  } else { return false;} // if not, return error
//return true;
}